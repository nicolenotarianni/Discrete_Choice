#this is the prep code for 10 randoms
...
for(i in 1:length(unique(ewpw$Bird.ID))){ # For each individual bird...
  ...
  for(j in 1:nrow(ewpw_i)){ # For each GPS point/choice ID...
    ...
    points1 = st_sample(buff2, size = 9) # generate 2 random points for each "used" point using st_sample()
...
write.csv(BigDF, "./FakeEWPWPtsForDCw10rand.csv", row.names = FALSE)
